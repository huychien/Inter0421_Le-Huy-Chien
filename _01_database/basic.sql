...
/*Câu 18. Xóa những khách hàng có hợp đồng trước năm 2016 (chú ý ràngbuộc giữa các bảng).*/
...
ALTER TABLE HopDong 
DROP FOREIGN KEY HopDong_fk_2;

ALTER TABLE HopDong 
ADD CONSTRAINT HopDong_fk_2
...
delete 
from KhachHang
where IdKhachHang in (select hd.IdKhachHang
					  from HopDong hd
                      where year(NgayKetThuc) < 2016);
                      
/*Câu 19. Cập nhật giá cho các Dịch vụ đi kèm được sử dụng trên 10 lần trong năm 2019 lên gấp đôi.*/

update DichVuDiKem
Set Gia = Gia * 2
where IdDichVuDiKem in (select hdct.IdDichVuDiKem  
						from HopDongChiTiet hdct join DichVuDiKem dvdk  on HopDong.IdHopDong = hdct.IdHopDong
                        where year(NgayLamHopDong) = 2019
						group by hdct.IdDichVuDiKem
						having count(hdct.IdDichVuDiKem) > 10);

/*Câu 20. Hiển thị thông tin của tất cả các Nhân viên và Khách hàng có trong hệ thống, thông tin hiển thị bao 
